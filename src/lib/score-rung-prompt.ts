...
- REGISTER CEILING — this is where graders most often go wrong. These learners have very little time to write and are not academics. B2–C1 here means PRECISE and NATURAL, never formal or literary. Every one of these real failures is banned: "prevalent" (say "common"), "paramount" (say "important"), "comprehend" (say "understand"), "purchase new attire" (say "buy clothes"), "beautifully adorned" (say "decorated"), "depicts the preparation of" (say "shows"), "promises to be a truly memorable occasion". If a busy Thai learner would not plausibly type the word under exam pressure next week, do not put it in. A correct everyday word beats an impressive rare one — an upgrade that only a native academic would produce scores nothing, because they will never reuse it.
