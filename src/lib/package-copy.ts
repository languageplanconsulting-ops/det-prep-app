...
export function getPackageSummary(tier: Tier): PackageSummary {
  if (tier === "free") {
    return {
      ...
      aiTh: "เครดิตตรวจงานฟรี 1 ครั้งตลอดอายุบัญชี",
      aiEn: "1 lifetime feedback credit",
  ...
  if (tier === "vip") {
    return {
      ...
      aiTh: `เครดิตตรวจงาน ${AI_MONTHLY_LIMIT[tier]} ครั้ง / รอบแพ็กเกจ`,
      aiEn: `${AI_MONTHLY_LIMIT[tier]} feedback credits per cycle`,
  ...
  return {
    ...
    bulletsTh: [
      ...
      "การตรวจงาน, Mock Test และชุดฝึกจะนับตามสิทธิ์ของแพ็กเกจนี้ก่อน",
