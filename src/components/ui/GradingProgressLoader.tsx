...
export function GradingProgressLoader({ eyebrow }: GradingProgressLoaderProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-center justify-center bg-neutral-900/35 p-4"
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
      <div className="ep-brutal w-full max-w-lg border-4 border-black bg-white p-6 shadow-[8px_8px_0_0_#000]">
        {eyebrow ? (
          <p className="ep-stat text-center text-[11px] font-bold uppercase tracking-[0.25em] text-ep-blue">
            {eyebrow}
          </p>
        ) : null}

        <div className="mt-4 flex flex-col items-center border-b-2 border-black pb-4">
        ...
        <div className="mt-5 h-3 overflow-hidden border-2 border-black bg-neutral-200">
          <div
            className="h-full bg-ep-blue transition-[width] duration-200 ease-out"
            style={{ width: `${Math.min(100, progress)}%` }}
          />
        </div>

        <div className="mt-6 min-h-[8rem] space-y-3 text-center">
          <p
            key={`th-${msgIndex}`}
        ...
        <p className="ep-stat mt-5 text-center text-[10px] font-bold text-neutral-400">
