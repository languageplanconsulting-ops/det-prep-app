...
function Player({ tier, unit, items, uid }: { tier: ReadWriteTier; unit: number; items: ReadWriteItem[]; uid: string | null }) {
  ...
  const allFilled = typed.length === fillWords.length && fillWords.every((w, i) => fitbRemainderLength(w) === 0 || (typed[i] ?? "").length > 0);
  const fillMarks = fillRevealed ? scoreFitb(fillWords.map((w, i) => fitbPrefix(w) + (typed[i] ?? "")), fillWords).marks : undefined;
  ...
  function checkFill() {
    markItemSeen(uid, itemKey("readwrite", item.id), "readwrite", "manual_browse").catch(() => {});
    const full = fillWords.map((w, i) => fitbPrefix(w) + (typed[i] ?? ""));
  ...
  function retryFill() {
    const full = fillWords.map((w, i) => fitbPrefix(w) + (typed[i] ?? ""));
    const marks = scoreFitb(full, fillWords).marks;
    setTyped((p) => p.map((v, i) => (marks[i]?.grade === "exact" ? v : "")));
    setFillRevealed(false);
  }
  ...
  return (
    <div key={index} className="ep-step-slide-in">
      ...
      {phase === "cloze" ? (
        isFill ? (
          <>
            <p className="mb-2 rounded-xl bg-blue-50 p-3 text-xs font-semibold text-[#004AAD]">อ่านเรียงความตัวอย่าง แล้วพิมพ์คำที่หายไป (มีตัวอักษรขึ้นต้นให้) — ฝึกคำที่ใช้คู่กัน การผันกริยา และคำเชื่อม</p>
              ...
              <div className="flex flex-wrap items-center gap-y-3 text-[15px] leading-8 text-slate-800">
                {splitTemplate(item.template).map((part, pi) =>
                  "text" in part ? (
                    ...
                    <span key={pi} className="mx-0.5 inline-flex items-center gap-0.5">
                      <span className="font-bold text-[#004AAD]">{fitbPrefix(fillWords[part.blank]!)}</span>
                      <input
                        value={typed[part.blank] ?? ""}
                        disabled={fillRevealed}
                        onChange={(e) => setTyped((p) => { const n = [...p]; n[part.blank] = e.target.value.slice(0, fitbRemainderLength(fillWords[part.blank]!)); return n; })}
                        className={`w-16 rounded-md border-b-2 bg-transparent px-1 text-center font-bold outline-none ${
                          fillMarks?.[part.blank]?.grade === "exact" ? "border-emerald-500 text-emerald-700" : fillMarks?.[part.blank] ? "border-rose-500 text-rose-700" : "border-[#004AAD]"
                        }`}
                        style={{ width: `${Math.max(2.5, fitbRemainderLength(fillWords[part.blank]!) * 0.75)}em` }}
                      />
                    </span>
                  ),
                )}
            ...
            {fillRevealed && fillMarks?.some((m) => m.grade !== "exact") ? (
              <div className="mt-4 rounded-xl border border-rose-500 bg-rose-50 p-4">
                <p className="mb-2 text-sm font-black text-rose-700">✕ ยังไม่ครบ — ดูคำที่ยังไม่ถูก แล้วเก็บลงสมุด</p>
                {item.blanks.map((b, i) => {
                  if (fillMarks?.[i]?.grade === "exact") return null;
                  const grammar = b.kind === "grammar";
          ...
          <>
            <p className="mb-2 rounded-xl bg-blue-50 p-3 text-xs font-semibold text-[#004AAD]">อ่านเรียงความตัวอย่าง แล้วแตะช่องว่างเพื่อเลือกคำและเครื่องหมายที่ถูก</p>
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-6">
        {phase === "review" ? (
          ...
          fillRevealed ? (
            <button type="button" onClick={retryFill} className="w-full rounded-xl bg-[#004AAD] py-3 text-sm font-bold text-[#FFCC00]">
              แก้แล้วลองใหม่
            </button>
          ) : (
            <button type="button" disabled={!allFilled} onClick={checkFill} className="w-full rounded-xl bg-[#004AAD] py-3 text-sm font-bold text-[#FFCC00] disabled:opacity-40">
              {allFilled ? "ตรวจคำตอบ" : "พิมพ์คำให้ครบก่อน"}
            </button>
          )
        ) : (
