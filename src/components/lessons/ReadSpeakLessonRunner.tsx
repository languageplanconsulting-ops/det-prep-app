...
import { readSpeakUnit, type ReadSpeakItem, type ReadSpeakTier, type ReadSpeakVocab } from "@/lib/readspeak-lessons";
import { LessonRecorder } from "@/components/lessons/LessonRecorder";
import { ReadSpeakPatternCoach, ReadSpeakPatternTeaser } from "@/components/lessons/ReadSpeakPatternCoach";
import { useEffectiveTier } from "@/hooks/useEffectiveTier";
import { OverlayBackdrop } from "@/components/ui/OverlayBackdrop";

const TOPIC = "readspeak";
type Phase = "cloze" | "speak";

export function ReadSpeakLessonRunner({
  tier,
  unit,
  onDone,
}: {
  ...
  const items = filterUnseen(readSpeakUnit(tier, unit), (l) => itemKey("readspeak", l.id), seenKeys);
  if (!items.length) {
    return (
...
function Player({
  ...
  // The VIP build-the-answer drill is derived, never stored as a phase: the tier
  // resolves asynchronously, so a phase set on mount would be computed before
  // `isVip` is known and would never correct itself.
  ...
  const [vocabOpen, setVocabOpen] = useState<ReadSpeakVocab | null>(null);
  ...
  async function onRecording(r: { base64: string; mimeType: string }) {
    ...
    setResult(null);
    markItemSeen(uid, itemKey("readspeak", item.id), "readspeak", "manual_browse").catch(() => {});
    try {
  ...
  async function saveVocab(v: ReadSpeakVocab) {
    if (!uid || savedWords.has(v.word)) return;
    try {
      await addNotebookEntry({ source: "speaking-read-and-speak", categoryIds: ["vocabulary"], titleEn: v.word, titleTh: v.th, bodyEn: v.en, bodyTh: v.th, userNote: "" });
  ...
  if (finished) {
    ...
    return (
      <div className="py-8">
        <CelebrateMascot
          title="เยี่ยมมาก!"
          subtitle="คุณได้ฝึกทั้งไวยากรณ์และการออกเสียงแล้ว ลองพูดหัวข้อของคุณเองด้วยโครงประโยคนี้ดูนะ"
        />
        <div className="mx-auto mt-6 w-full max-w-xs rounded-2xl bg-slate-50 p-6 text-center">
          ...
          <p className="mt-1 text-sm text-slate-600">ผ่านครบ {passedCount} จาก {total} หัวข้อ</p>
        </div>
  ...
  const showPattern = isVip && !!item.moves?.length && !patternDone && phase === "cloze";
  ...
  return (
    <div key={index} className="ep-step-slide-in">
      <div className="mb-4 flex items-center justify-between text-xs font-bold text-slate-500">
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${phase === "speak" ? "bg-[#004AAD] text-white" : "bg-slate-100 text-slate-500"}`}>
            {showPattern ? "1 · ประกอบคำตอบ" : phase === "cloze" ? "2 · เติมคำ" : "3 · ออกเสียง"}
          </span>
        </div>
      ...
      <div className="mb-4 rounded-2xl bg-slate-900 p-4 text-white">
        <p className="text-[10px] font-black uppercase tracking-wide text-[#FFCC00]">โจทย์พูด</p>
        <p className="mt-1.5 text-sm font-bold leading-relaxed">{item.topic}</p>
        <p className="mt-1 text-xs text-slate-300">{item.topicTh}</p>
      </div>

      {showPattern ? (
        <ReadSpeakPatternCoach item={item} onDone={() => setPatternDone(true)} />
      ) : phase === "cloze" ? (
        <>
          {!isVip && item.moves?.length ? <ReadSpeakPatternTeaser item={item} /> : null}
          <p className="mb-2 rounded-xl bg-blue-50 p-3 text-xs font-semibold text-[#004AAD]">อ่านคำตอบตัวอย่าง แล้วแตะช่องว่างเพื่อเลือกคำที่ถูก</p>
          ...
          <div className="mt-6 flex gap-2">
            {passed ? (
              <button type="button" onClick={next} className="w-full rounded-xl bg-slate-900 py-3 text-sm font-bold text-white">
                {index + 1 >= total ? "ดูสรุป →" : "หัวข้อถัดไป →"}
              </button>
            ) : (
