...
export function WriteAboutPhotoExamCard({ item }: { item: PhotoSpeakItemWithProgress }) {
  ...
  const sessionHref = `/practice/production/write-about-photo/play?itemId=${encodeURIComponent(item.id)}`;
        ...
        <div className="mt-auto border-t-2 border-neutral-200 p-3">
          {!started ? (
            ...
            <div className="flex flex-col gap-2">
              ...
              {showRedeem && progress ? (
                <Link
                  href={`/practice/production/write-about-photo/report/${progress.latest_attempt_id}`}
