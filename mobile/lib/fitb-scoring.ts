import type { FitbBlankGrade, FitbDifficulty, FitbMissingWord } from "./types";
import { fitbMaxScore } from "./fitb-constants";
...
function blankPointsForGrade(grade: FitbBlankGrade): number {
...
export function calculateFitbDetScore(args: {
  ...
  let sum = 0;
  for (let i = 0; i < n; i++) {
    const base = per * blankPointsForGrade(grades[i]!);
